...
{
	char *result;
	printc("try to get globaltable\n");
	printc("test global in scopetest=%d\n", scopetest);
	lua_getglobal(L, table_name);
	printc("check to see if it is actually a table");
	if(!lua_istable(L, -1))
	{
		printc("tablespace didn't exist so try to create\n");
		create_tablespace(L, table_name);
	}
	printc("looks like we should have our table now\n");
